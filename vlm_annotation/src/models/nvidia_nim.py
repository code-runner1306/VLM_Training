...
class NvidiaVisionModel(VisionModel):
    """Provider implementation for NVIDIA NIM Hosted Vision Models."""

    def __init__(self, provider_name: str, model_id: str, config: Dict[str, Any]):
        super().__init__(provider_name, model_id, config)
        if not HAS_OPENAI:
            raise ImportError("openai package is required for NvidiaVisionModel.")
